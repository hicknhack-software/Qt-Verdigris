...
#define DECLARE_TEST(NAME) void NAME(); W_SLOT(NAME)
...
#if QT_VERSION >= QT_VERSION_CHECK(6,5,0)
    DECLARE_TEST(qobjectBindableReallocatedBindingStorage);
#endif
...
#if QT_VERSION >= QT_VERSION_CHECK(6,5,0)
    DECLARE_TEST(bindingGroupMovingBindingData);
    DECLARE_TEST(bindingGroupBindingDeleted);
#endif
    ...
    DECLARE_TEST(notifyAfterAllDepsGone);
#endif
#if QT_VERSION >= QT_VERSION_CHECK(6,5,0)
    DECLARE_TEST(propertyAdaptorBinding);
#endif
...
{
    W_OBJECT(PropertyAdaptorTester)

signals:
    void fooChanged(int newFoo) W_SIGNAL(fooChanged, (int), newFoo);

public slots:
    void fooHasChanged() { fooChangedCount++; } W_SLOT(fooHasChanged);
...
public:
    ...
    W_PROPERTY(int, foo1 READ foo WRITE setFoo);
};
W_OBJECT_IMPL(PropertyAdaptorTester)
